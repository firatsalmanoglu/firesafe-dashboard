...
type ProviderList = Providers & 
                  { institution: PInstitutions } & 
                  { role: Roles };
...
const renderRow = (item: ProviderList) => (
  <tr
    key={item.id}
  ...
  >
    ...
    <td>
      <div className="flex items-center gap-2">
        <Link href={`/list/providers/${item.id}`}>
         ...
         {role === "admin" && (
          ...
          // </button>
          <FormModal table="provider" type="delete" id={item.id}/>
        )} 
      </div>
    </td>
  </tr>
...
const ProviderListPage = async ({
  ...
  const query: Prisma.ProvidersWhereInput = {}; // Prisma için boş bir query nesnesi oluşturuluyor.
    ...
    return (
        <div className='bg-white p-4 rounded-md flex-1 m-4 mt-0'>
            ...
            <div className='flex item-center justify-between'>
                <h1 className="hidden md:block text-lg font-semibold">Tüm Hizmet Sağlayıcılar</h1>
                <div className='flex flex-col md:flex-row items-center gap-4 w-full md:w-auto'>
                    ...
                    <div className="flex items-center gap-4 self-end">
                        ...
                        // </button>
                        <FormModal table="provider" type="create" />
                        )}
                    </div>
                </div>
            </div>
