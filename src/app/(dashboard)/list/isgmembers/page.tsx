    ...
    if (queryParams) {
      for (const [key, value] of Object.entries(queryParams)) {
        if (value !== undefined) {
          switch (key) {
            case "isgNumber":
              const isgNumber = parseInt(value); // value'yu tam sayıya çeviriyoruz.
              if (!isNaN(isgNumber)) { // geçerli bir sayı olup olmadığını kontrol ediyoruz.
    ...
    return (
        <div className='bg-white p-4 rounded-md flex-1 m-4 mt-0'>
            ...
            <div className='flex item-center justify-between'>
                <h1 className="hidden md:block text-lg font-semibold">Tüm ISG Sorumluları</h1>
